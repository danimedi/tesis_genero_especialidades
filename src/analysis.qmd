...
This graph contains the trends of the total population (global trend) and the trends of the different groups of specialties (surgical vs. clinical).

```{r}
ggplot(x) +
  geom_line(aes(year, women)) +
  geom_smooth(aes(year, women), method = "glm", method.args = list(family = "binomial"), se = FALSE, color = "blue") +
  geom_line(aes(year, women, color = especialidad_quirurgica)) +
  labs(x = "Año", y = "Proporción de mujeres")
```
...
NOTE: THE PART BELOW THIS LINE CONTAINS ANALYSIS THAT I HAVEN'T FINISHED YET
