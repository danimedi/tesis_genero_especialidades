...
y <- dat %>%
  ...
  group_by(year, especialidad) %>%
  summarize(odds_women = sum(genero, na.rm = TRUE) / sum(genero == 0, na.rm = TRUE), n = length(year))

p_women_global <- x %>% group_by(year) %>% summarize(women_global = sum(genero, na.rm = TRUE) / sum(genero == 0, na.rm = TRUE))
...
ggplot(y) +
  ...
  geom_line(aes(year, log(women_global)), size = 0.7) +
  scale_x_continuous(breaks = seq(2013, 2023, by = 1)) +
  labs(x = "Año", y = "Logaritmo de razón mujeres : varones", color = "Especialidad") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red")
ggsave(here("output/graph_7-8.png"))
```
