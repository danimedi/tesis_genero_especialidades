...
res <- vector("double", length(years))
...
for (k in seq_along(years)) {
  i <- dat$year == years[[k]]
  res[[k]] <- sum(!is.na(dat[i, "gender", drop = TRUE]))
}
data.frame(
  year = years,
  n = res
...
for (k in seq_along(years)) {
  i <- dat$year == years[k]
  total <- length(dat[i, "gender", drop = TRUE])
  with_gender <- sum(!is.na(dat[i, "gender", drop = TRUE]))
  res[[k]] <- with_gender / total
}
data.frame(
  year = years,
  n = res
...
# Porcentaje de mujeres en los distintos años
i <- is.na(dat$gender)
(1 - mean(i)) * 100
...
for (k in seq_along(years)) {
  i <- dat$year == years[k]
  res[[k]] <- mean(dat[i, "gender", drop = TRUE], na.rm = TRUE)
}
data.frame(
  year = years,
  n = res
...
```{r}
specialties <- unique(dat$especialidad_postulantes)
res_n <- vector("double", length(specialties))
res_p_women <- vector("double", length(specialties))
for (k in seq_along(specialties)) {
  i <- dat$especialidad_postulantes == specialties[k]
  res_n[[k]] <- nrow(dat[i, ])
  res_p_women[[k]] <- mean(dat[i, "gender", drop = TRUE], na.rm = TRUE)
}
data.frame(
...
# Detect the missing specialties
i <- is.na(dat$especialidad_postulantes)
dat[i, ] %>% group_by(year) %>% count()

x <- dat %>% group_by(especialidad_postulantes) %>% count()
y <- dat %>%
  group_by(especialidad_postulantes) %>%
  summarize(p_women = mean(gender, na.rm = TRUE))
left_join(x, y, by = join_by(especialidad_postulantes))
```
...
```{r}
model <- glm(gender ~ year, data = dat[!i_no_gender, ], family = "binomial")
summary(model)
...
```{r}
model_1 <- glm(gender ~ year, data = dat[!i_no_gender, ], family = "binomial")
summary(model_1)
exp(coef(model_1))
exp(confint(model_1))
```
...
```{r}
model_2 <- glm(gender ~ year + especialidad_quirurgica, data = dat[!i_no_gender, ], family = "binomial")
summary(model_2)
...
```{r}
specialties <- unique(dat[!i_no_gender, ]$especialidad_postulantes)
...
```{r}
model_3 <- glm(gender ~ year + especialidad_quirurgica, data = dat[!i_no_gender, ], family = "binomial")
summary(model_2)
```
