...
También se podría incluir la asignación de género como una posible variable confusora.

```{r}
x <- dat
x$no_asigned_gender <- ifelse(is.na(x$genero), 1, 0)
model <- glm(genero ~ year + region + especialidad + no_asigned_gender, family = "binomial", data = x)
model_summary <- summary(model)
```
