...
ggplot(select_biggest(x_women), aes(year, women, color = especialidad)) +
  geom_line() +
  scale_x_continuous(breaks = seq(2013, 2023, by = 1)) +
  labs(x = "Año", y = "Proporción de mujeres", color = "Especialidad")

ggplot(select_biggest(x_men), aes(year, women, color = especialidad)) +
  geom_line() +
  scale_x_continuous(breaks = seq(2013, 2023, by = 1)) +
  labs(x = "Año", y = "Proporción de mujeres", color = "Especialidad")
```
...
y <- dat %>%
  ...
  group_by(year, especialidad) %>%
  summarize(women = mean(genero, na.rm = TRUE), n = length(year))

p_women_global <- x %>% group_by(year) %>% summarize(women_global = mean(genero, na.rm = TRUE))

y <- left_join(y, p_women_global, by = c("year"))

ggplot(y) +
  geom_line(aes(year, women, color = especialidad)) +
  geom_point(aes(year, women, color = especialidad), alpha = 0.1) +
  geom_line(aes(year, women_global))
...
# specialties[i]
# 
# list_models_summary[ifelse(is.na(i), FALSE, i)]
# 
# # or_values <- conf_int_values <- vector("list", length(list_models))
# # for (i in seq_along(list_models)) {
# #   or_values[[i]] <- exp(coef(list_models[[i]]))[2] # OR of specialty
...
ggplot(y) +
  ...
  geom_line(aes(year, log(women_global)))

ggplot()
```

```{r}
i <- dat %>% group_by(especialidad) %>% count()
i <- i[i$n > 3000, 1, drop = TRUE]

y <- dat %>%
  filter(!is.na(genero), especialidad %in% i) %>%
  group_by(year, especialidad) %>%
  summarize(odds_women = sum(genero, na.rm = TRUE) / sum(genero == 0, na.rm = TRUE), n = length(year))
