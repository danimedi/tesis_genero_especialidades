...
x <- x %>%
  group_by(year, especialidad) %>%
  summarize(p_mujeres = mean(genero, na.rm = TRUE))
p_years <- dat %>% group_by(year) %>% summarize(p_total = mean(genero, na.rm = TRUE))
x <- left_join(x, p_years, by = "year")

ggplot(x) +
  geom_line(aes(year, p_total), size = 0.7) +
  geom_line(aes(year, p_mujeres, color = especialidad)) +
  scale_x_continuous(breaks = seq(2013, 2023, by = 1)) +
...
Antes de realizar los modelos de regresión logística, debemos verificar que la relación entre el año y los log-odds del género es lineal, para esto podemos realizar un gráfico.

```{r}
# Obtain the quantiles from the year to slice the data in 4 groups
q <- quantile(dat$year, probs = c(0, 0.25, 0.5, 0.75, 1))

# Use this quantiles to create categories of the years
year_cat <- cut(dat$year, breaks = q, right = FALSE, include.lowest = TRUE)

# Find the probabilities (proportions) for all of these categories
probs <- split(dat$genero, year_cat)
probs <- vapply(probs, mean, double(1), na.rm = TRUE)
# and calculate the log-odds
logits <- log(probs / (1 - probs))

# Find de medians of the years
meds <- split(dat$year, year_cat)
meds <- vapply(meds, median, double(1), na.rm = TRUE)

# Generate a plot to check for linearity
plot(
  meds, logits,
  main = "Should look linear if year is linear with log-odds of gender",
  xlab = "Year", ylab = "Log-odds(Gender)",
  las = 1
)
```
...
Ahora, podemos separar las especialidades con diferencias significativas de acuerdo el efecto en el género (más hombres o más mujeres), de acuerdo al odds ratio.

```{r}
# Separate by odds ratio ...
x <- df_res[df_res$p_value_significant, ]
more_women <- x[x$or_values > 1, ]
more_men <- x[x$or_values < 1, ]

biggest_specialties <- function(threshold = 20) {
  x <- dat %>% group_by(especialidad) %>% count()
  i <- x$n > threshold
  x[i, 1, drop = TRUE]
}
# 
# n <- 100
# i <- more_women$Especialidad %in% biggest_specialties(threshold = n)
j <- more_women$Especialidad %in% unique(dat$especialidad[dat$subespecialidad == 0])
write.table(
  arrange(more_women[j, ], desc(or_values)),
  here("output/tabla_3-mas_mujeres.txt"),
  sep = "\t", row.names = FALSE
)


j <- more_men$Especialidad %in% unique(dat$especialidad[dat$subespecialidad == 0])
write.table(
  arrange(more_men[j, ], or_values),
  here("output/tabla_3-mas_hombres.txt"),
  sep = "\t", row.names = FALSE
)
```
...
```{r graph-1-2}
x_women <- dat %>%
  filter(!is.na(genero), especialidad %in% more_women$Especialidad) %>%
  group_by(year, especialidad) %>%
  summarize(women = mean(genero), n = length(year))

x_men <- dat %>%
  filter(!is.na(genero), especialidad %in% more_men$Especialidad) %>%
  group_by(year, especialidad) %>%
  summarize(women = mean(genero), n = length(year))

# ... select the specialties with more applicants
select_biggest <- function(df, n = 6) {
  y <- df %>% group_by(especialidad) %>% summarize(n = sum(n))
  i <- order(y$n, decreasing = TRUE)[1:n]
  j <- y[i, "especialidad", drop = TRUE]
  df[df$especialidad %in% j, ]
}

# "Global" data
p_years <- dat %>% group_by(year) %>% summarize(p_total = mean(genero, na.rm = TRUE))
x_women <- left_join(x_women, p_years, by = c("year"))
x_men <- left_join(x_men, p_years, by = c("year"))

ggplot(select_biggest(x_women)) +
  geom_line(aes(year, p_total), size = 0.7) +
  geom_line(aes(year, women, color = especialidad)) +
  scale_x_continuous(breaks = seq(2013, 2023, by = 1)) +
  labs(x = "Año", y = "Proporción de mujeres", color = "Especialidad") +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red") +
  theme(legend.position = "bottom")
ggsave(here("output/graph_1-2-mujeres.png"))

ggplot(select_biggest(x_men)) +
  geom_line(aes(year, p_total), size = 0.7) +
  geom_line(aes(year, women, color = especialidad)) +
  scale_x_continuous(breaks = seq(2013, 2023, by = 1)) +
  labs(x = "Año", y = "Proporción de mujeres", color = "Especialidad") +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red") +
  theme(legend.position = "bottom")
ggsave(here("output/graph_1-2-hombres.png"))
```
...
x$genero_factor <- factor(x$genero_factor, levels = c("No sexo asignado", "Masculino", "Femenino"))

ggplot(x) +
  geom_bar(aes(year, fill = genero_factor)) +
  labs(x = "Año", y = "Número de postulantes", fill = "Sexo") +
  scale_fill_manual(values = c("No sexo asignado" = "grey80", "Masculino" = "#619CFF", "Femenino" = "#F8766D"))
...
specialties <- unique(x$especialidad)
list_models <- lrt_result_p_value <- vector("list", length(specialties))
for (i in seq_along(specialties)) {
  model_raw <- glm(
    genero ~ year + ifelse(especialidad == specialties[[i]], 1, 0),
    data = x
  )
  ...
  lrt_result_p_value[[i]] <- anova(model_raw, model_interaction, test = "LRT")[2, 5]
  list_models[[i]] <- model_interaction
}
list_models_summary <- lapply(list_models, summary)
names(list_models_summary) <- specialties
list_models_summary[1]
lrt_result_p_value[1]

p_value_corrected <- 0.05 / length(specialties)
i <- unlist(lrt_result_p_value) < p_value_corrected
specialties_significant <- specialties[i]
specialties_significant <- specialties_significant[!is.na(specialties_significant)]

# Make a graph for those specialties
y <- dat %>%
  filter(!is.na(genero), especialidad %in% specialties_significant) %>%
  group_by(year, especialidad) %>%
  summarize(
    odds_women = sum(genero, na.rm = TRUE) / sum(genero == 0, na.rm = TRUE),
    p_women = mean(genero, na.rm = TRUE),
    n = length(year)
  )
...
p_women_global <- x %>%
  group_by(year) %>%
  summarize(
    odds_women_global = sum(genero, na.rm = TRUE) / sum(genero == 0, na.rm = TRUE),
    p_women_global = mean(genero, na.rm = TRUE)
  )

y <- left_join(y, p_women_global, by = c("year"))

ggplot(y) +
  geom_line(aes(year, log(odds_women), color = especialidad)) +
  geom_point(aes(year, log(odds_women), color = especialidad), alpha = 0.1) +
  geom_line(aes(year, log(odds_women_global)), size = 0.7) +
  scale_x_continuous(breaks = seq(2013, 2023, by = 1)) +
  labs(x = "Año", y = "Logaritmo de razón mujeres : varones", color = "Especialidad") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red")
ggsave(here("output/graph_7-8-logs_odds.png"))

ggplot(y) +
  geom_line(aes(year, p_women, color = especialidad)) +
  geom_point(aes(year, p_women, color = especialidad), alpha = 0.1) +
  geom_line(aes(year, p_women_global), size = 0.7) +
  scale_x_continuous(breaks = seq(2013, 2023, by = 1)) +
  labs(x = "Año", y = "Proporción de mujeres", color = "Especialidad") +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red") +
  theme(legend.position = "bottom")
ggsave(here("output/graph_7-8-proportion_women.png"))
```
...
x <- dat[!is.na(dat$genero), ]
model <- glm(genero ~ year + especialidad_quirurgica, family = "binomial", data = x)
model_summary <- summary(model)
...
```{r}
x <- filter(dat_ingresantes, subespecialidad == 0) # Remove subspecialties
res <- x %>%
  group_by(especialidad) %>%
  summarize(
    n_postulantes = length(especialidad),
...
write.table(
  arrange(res, p_mujeres),
  here("output/tabla_6-mas_hombres.txt"), sep = "\t", row.names = FALSE
)
write.table(
  arrange(res, p_hombres),
  here("output/tabla_6-mas_mujeres.txt"), sep = "\t", row.names = FALSE
)
```
...
ggplot(x) +
  geom_bar(aes(year, fill = genero_factor)) +
  labs(x = "Año", y = "Número de ingresantes", fill = "Sexo") +
  scale_fill_manual(values = c("No sexo asignado" = "grey80", "Masculino" = "#619CFF", "Femenino" = "#F8766D"))
...
Primero realizar un modelo estadístico simple para comprobar que el año de postulación es aún estadísticamente significativo como predictor del género entre los ingresantes solamente.
...
Análisis estadístico de especialidades clínicas vs. quirúrgicas.

```{r}
# ---- without effect modification ----
x <- dat[!is.na(dat$genero) & dat$year >= 2016, ]
x <- x[x$ingreso == 1, ]
model <- glm(genero ~ year + especialidad_quirurgica, family = "binomial", data = x)
model_summary <- summary(model)
round(cbind(exp(coef(model)), exp(confint(model))), 2)
...
x$genero_factor <- factor(
  x$genero_factor,
  levels = c("No sexo asignado", "Masculino", "Femenino")
)

x$region <- factor(
  x$region,
  levels = c("Lima", "Norte", "Sur", "Centro", "Oriente")
)

ggplot(x) +
  geom_bar(aes(region, fill = genero_factor)) +
  labs(x = "Región", y = "Número de postulantes", fill = "Sexo") +
  scale_fill_manual(values = c("No sexo asignado" = "grey80", "Masculino" = "#619CFF", "Femenino" = "#F8766D"))
...
p_data <- p_data[!is.na(p_data$region), ] # Removing missing values
...
x <- dat[!is.na(dat$genero), ]
x <- x[!is.na(x$region), ] # Remove missing regions
...
x <- dat[!is.na(dat$genero), ]
regions <- unique(x$region)
regions <- regions[!is.na(regions)] # remove NAs
list_models <- vector("list", length(regions))
...
También se podría hacer un análisis del peor o mejor caso (todos los datos faltantes que sean mujeres o todos los datos faltantes que sean hombres).

```{r}
# Worst and best case analysis
x <- dat
x$genero <- ifelse(is.na(x$genero), 1, x$genero) # 1 or 0
model <- glm(genero ~ year + region + especialidad, family = "binomial", data = x)
model_summary <- summary(model)
model_summary
```
