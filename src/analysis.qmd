...
tab <- x %>% 
  tbl_summary(
    by = genero2, missing_text = "Missing", percent = "row",
    digits = list(
      year2 ~ c(0, 1),
      ingreso ~ c(0, 1),
      especialidad_tipo ~ c(0, 1),
...
tab <- x %>% 
  tbl_summary(
    by = ingreso, missing_text = "Missing", percent = "row",
    digits = list(
      year2 ~ c(0, 1),
      genero2 ~ c(0, 1),
      especialidad_tipo ~ c(0, 1),
...
Usando al resultado de postulación (ingreso) como variable de resultado ...

```{r}
x <- na.omit(x)
x$region <- relevel(x$region, "Lima")
...
Y usando a sexo como variable de resultado ...
...
saveRDS(CIM_vacio, here("output/model_CIM_vacio.rds"))
...
saveRDS(CIM_simple, here("output/model_CIM_simple.rds"))
...
saveRDS(CIM_completo, here("output/model_CIM_completo.rds"))
